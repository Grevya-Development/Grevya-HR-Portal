...
export const useStore = create<AppState>((set, get) => ({
  ...
  fetchInitialData: async () => {
    try {
      ...
      set({ employees, leaveRequests, notifications });
    } catch (e) {
      console.error(e);
    }
  },
  ...
  markNotificationRead: async (id) => {
    ...
    set(s => ({ notifications: s.notifications.map(n => n.id === id ? { ...n, isRead: 1 as any } : n) }));
  },

  markAllRead: async () => {
    await fetch(`${API_URL}/notifications/mark-all-read`, { method: 'PUT', headers: getHeaders() });
    set(s => ({ notifications: s.notifications.map(n => ({ ...n, isRead: 1 as any })) }));
  },
}));
